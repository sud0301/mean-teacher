...
import resnext101_64x4d_features
import resnext101_32x4d_features
...
def resnext101_64x4d(num_classes=1000, pretrained='imagenet'):
    ...
    if pretrained is not None:
        ...
        assert num_classes == settings['num_classes'], \
            "num_classes should be {}, but is {}".format(settings['num_classes'], num_classes)
        #model.load_state_dict(model_zoo.load_url(settings['url']))
        model.load_state_dict(torch.load('/misc/lmbraid19/mittal/dense_prediction/cloned/mean-teacher/pytorch/mean_teacher/resnext_101_64x4d.pth'))
        model.input_space = settings['input_space']
        model.input_size = settings['input_size']
        model.input_range = settings['input_range']
        model.mean = settings['mean']
